...
public class DataToDrawProvider {
    ...
    private int wfsAddResult(DataToDraw data, WfsResponse response)
    {
        ...
        {
            for ( int j=0 ; j < response.getMembers().get(i).getBoundaries().size() ; j++ )
            {
                for ( int k=0 ; k < response.getMembers().get(i).getBoundaries().get(j).getCurves().size() ; k++ )
                {
                    for ( int l=0 ; l < response.getMembers().get(i).getBoundaries().get(j).getCurves().get(k).getLineStrings().size() ; l++ )
                    {
                        List<Double> listLatLon = response.getMembers().get(i).getBoundaries().get(j).getCurves().get(k).getLineStrings().get(l).getParsedPosList();
    ...
    /**
     * Read wfs params from config file
     * @param context
     * @return
     */
    /*private boolean getWFSUser(Context context)
    {
        try {
            InputStream is = context.getAssets().open(file_wfs_matrikkel_user);
            int size = is.available();

            Scanner s = new Scanner(is);
            s.useLocale(Locale.US);

            _wfs_user = s.nextLine();
            _wfs_pass = s.nextLine();
            _wfs_url = s.nextLine();
        ...
        return ( _wfs_user.length() > 0 && _wfs_pass.length() > 0 );
    }*/
